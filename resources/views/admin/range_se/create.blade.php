...
@section('title', 'Tambah Range Kategori SE')

@section('content_header')
    <h1>Tambah Range Kategori SE</h1>
@endsection

@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('rangese.store') }}" method="POST">
                @csrf

                <div class="form-group">
                    <label for="nilai_akhir_aset">Nilai Akhir Aset</label>
                    <input type="text" name="nilai_akhir_aset" class="form-control" value="{{ old('nilai_akhir_aset') }}"
                        required>
                </div>

                <div class="form-group">
                    <label for="warna_hexa">Warna (Hex)</label>
                    <input type="color" name="warna_hexa" class="form-control" value="{{ old('warna_hexa', '#000000') }}"
                        required>
                </div>
